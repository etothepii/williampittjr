 ...
 * Time: 13:51
 */
public class UKIPLogoGeneratorImplTests {

    private static UKIPLogoGeneratorImpl ukipLogoGeneratorImpl;

    @BeforeClass
    public static void setUp() {
        try {
            ukipLogoGeneratorImpl = new UKIPLogoGeneratorImpl();
        } catch (IOException e) {
    ...
    public void toryStyleGuideExampleTest() {
        ...
        for (int i = 0; i < associationNames.length; i++) {
            logos[i] = new BufferedImage[] {
                ukipLogoGeneratorImpl.getLogo(associationNames[i]),
                ukipLogoGeneratorImpl.getPaddedLogo(associationNames[i])
            };
